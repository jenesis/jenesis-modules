...
public class StreamingBatchSourceTest {
    ...
    public void groups_queue_items_into_batches_up_to_size() throws InterruptedException {
        BlockingQueue<IndexStream.QueueItem> queue = new ArrayBlockingQueue<>(32);
        for (int i = 1; i <= 5; i++) {
            queue.put(new IndexStream.QueueItem(coordinate("g", "a", i + ".0"), i));
        }
    ...
    public void returns_empty_open_batch_when_queue_is_temporarily_empty() throws InterruptedException {
        BlockingQueue<IndexStream.QueueItem> queue = new ArrayBlockingQueue<>(4);
    ...
    public void marks_exhausted_when_poison_arrives_mid_batch() throws InterruptedException {
        BlockingQueue<IndexStream.QueueItem> queue = new ArrayBlockingQueue<>(8);
        queue.put(new IndexStream.QueueItem(coordinate("g", "a", "1.0"), 1));
        queue.put(new IndexStream.QueueItem(coordinate("g", "a", "2.0"), 2));
        queue.put(IndexStream.QueueItem.POISON);
