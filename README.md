...
- The file is absent when the module has no `named` rows for the resolved owner (e.g. the owner only ever shipped `automatic` JARs, `owners.tsv` filtered out every `named` claim, or every named row failed the version-equality filter), and also for **JDK-bundled module names** (see below). The crawler also deletes a stale `modules.tsv` when a regeneration ends with zero qualifying rows.
- **JDK-bundled module names are never emitted.** A module name that ships inside the JDK (`java.base`, `java.sql`, `java.xml`, `jdk.net`, `jdk.unsupported`, ...) gets no `modules.tsv` at all, even when some Maven coordinate declares it (a JDK reimplementation like `org.qbicc.rt:*`, an upgrade-module-path build like `org.graalvm.compiler:compiler`, etc.). Such a name can never resolve as that module on a standard JVM: the platform's own copy is found first on the module path and shadows anything supplied externally, so a `/module/<name>` lookup would only ever hand back a JAR the runtime refuses to use. The audit log (`versions.tsv`) keeps the declarations, and `artifacts.tsv` still serves the underlying JARs as plain coordinate-keyed downloads. The set (`ModuleStore.PLATFORM_MODULES`) is a fixed, explicit list of the `java --list-modules` platform modules, kept hardcoded rather than read from `ModuleFinder.ofSystem()` so the resolved views regenerate identically on every JDK (the platform set drifts between releases: `jdk.jsobject`, for instance, ships in JDK 25 but was dropped in JDK 26). Legacy Java EE modules that were **removed** from the JDK (JEP 320: `java.xml.bind`, `java.xml.ws`, `java.transaction`, `java.activation`, ...) are not bundled with a modern JVM and so are *not* in this set - they are legitimately republished on Maven Central and keep resolving - and JavaFX (`javafx.*`) is likewise not a JDK module.
...
One name-level rule runs ahead of owner resolution for `modules.tsv` only: **JDK-bundled module names** (every `java.*`/`jdk.*` module in a current `java --list-modules`, listed in `ModuleStore.PLATFORM_MODULES`) never get a `modules.tsv`, no matter who published them, because the JVM always shadows them on the module path so the resolution could not be used (see the `modules.tsv` section above). `artifacts.tsv` and `versions.tsv` are unaffected.
